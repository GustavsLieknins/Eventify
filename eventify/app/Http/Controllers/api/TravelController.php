...
class TravelController extends Controller
{
    public function flights(Request $request, HasDataClient $hasData)
    {
        ...
        if (!$arrivalId) {
            return response()->json([
                'error' => 'Missing or ambiguous destination. Provide one of: arrivalId, arrivalAirportCode, arrivalCityCode, or city + countryCode.',
                'hint'  => 'Examples: arrivalCityCode=LON for London UK; arrivalAirportCode=YXU for London, Ontario, Canada; or city=London&countryCode=GB/CA.',
            ], 422);
        }
        ...
        if (!$returnDate) {
            $returnDate = Carbon::parse($outboundDate)
                ->addDays(max(1, $stay))
                ->format('Y-m-d');
        }
    ...
    public function hotels(Request $request, HasDataClient $hasData)
    {
        ...
        if ($city !== '') {
            $fallbackCity = $hasData->mapsSearch([
                'q'  => "hotels in {$city}",
                'hl' => 'en',
            ]);
